<?php
/**
 * SSH functions and definitions
 *
 * @package SSH
 */

if ( ! function_exists( 'SSH_setup' ) ) :
/**
...
function SSH_setup() {
	 ...
	 * If you're building a theme based on SSH, use a find and replace
	 * to change 'SSH' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'SSH', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary Menu', 'SSH' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'SSH_custom_background_args', array(
		'default-color' => 'ffffff',
		'default-image' => '',
	) ) );
}
endif; // SSH_setup
add_action( 'after_setup_theme', 'SSH_setup' );
 ...
 */
function SSH_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'SSH_content_width', 640 );
}
add_action( 'after_setup_theme', 'SSH_content_width', 0 );
 ...
 */
function SSH_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'SSH' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
add_action( 'widgets_init', 'SSH_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function SSH_scripts() {
	wp_enqueue_style( 'SSH-style', get_stylesheet_uri() );
...
add_action( 'wp_enqueue_scripts', 'SSH_scripts' );
